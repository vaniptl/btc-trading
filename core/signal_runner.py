"""
signal_runner.py — Trinity.AI BTC Strategy Core Engine v8.5
FIXES:
  ✅ BEAR regime enable_longs=True (was False in broken version)
  ✅ BEAR long min_score lowered to 6.5 when override active (was 8.0 - too strict)
  ✅ get_recommendation() added — always shows buy/sell/hold guidance even with no signal
  ✅ Binance data-api.binance.vision endpoint (avoids geo-blocking)
  ✅ Intelligence layer graceful fallback — never blocks signal output
  ✅ save_to_memory() works correctly on all deployments
  ✅ log_trade_to_sheets() added (was missing causing AttributeError in bot.py)
"""
...
# ── Intelligence Layer — optional, graceful fallback ──────────────
...
# ══════════════════════════════════════════════════════════════════
# REGIME CONFIGS — BEAR enable_longs=True is CRITICAL
# ══════════════════════════════════════════════════════════════════
REGIME_CONFIGS = {
    "STRONG_BULL": {
    ...
    "BEAR": {
        "enable_longs": True,   # ✅ FIXED: must be True for MR longs in ranging overrides
        "enable_shorts": True,
        "min_score_long": 7.0,  # Hard gate — only fires with strong confluence
        "min_score_short": 4.0,
        ...
        "description": "BEAR — Shorts primary | MR longs at extremes if ranging", "emoji": "🐻",
    },
    "RANGING": {
        "enable_longs": True, "enable_shorts": True,
        "min_score_long": 5.5, "min_score_short": 5.5,
...
# ── Data endpoints ─────────────────────────────────────────────────
BINANCE_SPOT_URL  = "https://data-api.binance.vision/api/v3/klines"  # geo-block resistant
...
def fetch_ohlcv_binance(symbol="BTCUSDT", interval="1h", days_back=365):
    ...
    while cur < end_ms:
        try:
            ...
            last = batch[-1][0]
            cur  = last + ms_step
            if last >= end_ms - ms_step: break
            time.sleep(0.12)
        except Exception as e:
            print(f"[Binance] {interval} error: {e}"); break
    if not all_data: return pd.DataFrame()
    df = pd.DataFrame(all_data, columns=["open_time","open","high","low","close","volume","close_time","qv","nt","tbv","tbq","ig"])
...
def fetch_ohlcv_cc(unit="hour", aggregate=1, days_back=365):
    eps = {"hour": "https://min-api.cryptocompare.com/data/v2/histohour",
    ...
    while fetched < total_need:
        params = {"fsym": "BTC", "tsym": "USD", "limit": min(2000, total_need-fetched), "aggregate": aggregate}
...
def fetch_ohlcv(unit="hour", aggregate=1, days_back=365):
    ...
    print(f"[fetch_ohlcv] Binance failed for {interval} — CC fallback")
...
def fetch_liquidations(max_days=30):
    ...
    for _ in range(min(max_days, 7)):
        start_ms = end_ms - day_ms
        try:
            r = requests.get(url, params={"symbol":"BTCUSDT","startTime":start_ms,"endTime":end_ms,"limit":1000}, timeout=15)
            raw = r.json()
            if isinstance(raw, list): all_orders.extend(raw)
        except: pass
        end_ms = start_ms; time.sleep(0.1)
...
def fetch_funding():
    try:
        r = requests.get(f"{BINANCE_FAPI_URL}/fundingRate", params={"symbol":"BTCUSDT","limit":500}, timeout=15)
        raw = r.json()
...
def fetch_oi():
    try:
        r = requests.get(f"{BINANCE_FDATA_URL}/openInterestHist", params={"symbol":"BTCUSDT","period":"1h","limit":500}, timeout=15)
        raw = r.json()
        if not isinstance(raw, list) or not raw: return pd.DataFrame()
        df = pd.DataFrame(raw)
...
# ══════════════════════════════════════════════════════════════════
# VOLUME PROFILE / ANCHORED VWAP / BULL FLAG
# ══════════════════════════════════════════════════════════════════
...
def compute_volume_profile(df, bins=50, lookback=200):
    ...
    if pmax <= pmin:
        mid = (pmax+pmin)/2; return float(mid), float(pmax), float(pmin)
...
def compute_indicators(df, df_4h, df_1d, liq_df, funding_df, oi_df):
    ...
    safe=df["close"].replace(0, np.nan)
    df["near_poc"] =(df["close"]-poc).abs()/safe<0.008
    df["near_vah"] =(df["close"]-vah).abs()/safe<0.008
    df["near_val"] =(df["close"]-val).abs()/safe<0.008
    ...
    sh_idx=df["high"].iloc[-lb_av:].idxmax(); sl_idx=df["low"].iloc[-lb_av:].idxmin()
    ...
    df["avwap_bull_conf"]=df["avwap_low"].notna() &((df["close"]-df["avwap_low"]).abs()/safe<0.01)
    df["avwap_bear_conf"]=df["avwap_high"].notna()&((df["close"]-df["avwap_high"]).abs()/safe<0.01)
    ...
    df["near_fib382"] =(df["close"]-fib382).abs()/safe<0.008
    df["near_fib50"]  =(df["close"]-fib50).abs()/safe<0.008
    df["near_fib618"] =(df["close"]-fib618).abs()/safe<0.008
    df["near_fib100"] =(df["close"]-fib100).abs()/safe<0.008
    df["near_fib1618"]=(df["close"]-fib1618).abs()/safe<0.008
    ...
    for i in range(ob_window,len(df)):
        ...
        bi=w["close"].idxmin(); si=w["close"].idxmax()
        blo=float(df.loc[bi,"low"]); bhi=float(df.loc[bi,"high"])
        slo=float(df.loc[si,"low"]); shi=float(df.loc[si,"high"])
        cc=float(df["close"].iloc[i])
        if blo<=cc<=bhi: bull_ob.iloc[i]=True
        if slo<=cc<=shi: bear_ob.iloc[i]=True
    df["in_bullish_ob"]=bull_ob; df["in_bearish_ob"]=bear_ob
    ...
    for i in range(14,len(df)):
        ...
        if cv2[i]<cv2[p] and rsi_v2[i]>rsi_v2[p] and rsi_v2[i]<50: rb[i]=True
        if cv2[i]>cv2[p] and rsi_v2[i]<rsi_v2[p] and rsi_v2[i]>50: rr[i]=True
        if cv2[i]>cv2[p] and rsi_v2[i]>rsi_v2[p] and rsi_v2[i]<50: rhb[i]=True
        if cv2[i]<cv2[p] and rsi_v2[i]<rsi_v2[p] and rsi_v2[i]>50: rhr[i]=True
    df["rsi_bull_div"]=rb; df["rsi_bear_div"]=rr
    df["rsi_hidden_bull_div"]=rhb; df["rsi_hidden_bear_div"]=rhr
    is_up=df["close"]>df["ema200"]
    df["rsi_trend_bull_zone"]=is_up&(df["rsi"]>=30)&(df["rsi"]<=55)
    df["rsi_trend_bear_zone"]=(~is_up)&(df["rsi"]>=45)&(df["rsi"]<=70)

    if not df_4h.empty and len(df_4h)>=21:
        e8_4h=_ema(df_4h["close"],8); e21_4h=_ema(df_4h["close"],21); e55_4h=_ema(df_4h["close"],55)
    ...
    for i in range(20,len(df)):
        ph=max(highs_v[max(0,i-20):i]); pl_=min(lows_v[max(0,i-20):i])
        if close_vv[i]>ph: bos_up[i]=True
        if close_vv[i]<pl_: bos_dn[i]=True
    df["bos_up"]=bos_up; df["bos_down"]=bos_dn; df["choch"]=df["bos_up"]|df["bos_down"]
    ...
    if not funding_df.empty and "funding_rate" in funding_df.columns:
        fr=funding_df["funding_rate"].reindex(df.index,method="ffill")
        df["funding_extreme_long"] =fr<-0.0005
        df["funding_extreme_short"]=fr> 0.0005

    df["oi_confirm_long"]=False; df["oi_confirm_short"]=False
    if not oi_df.empty and "oi" in oi_df.columns:
        oi_s=oi_df["oi"].reindex(df.index,method="ffill"); oi_ch=oi_s.pct_change()
...
def detect_local_structure(df, lookback_bars=48, tf_label="1H"):
    ...
    e8=float(recent["ema8"].iloc[-1]); e21=float(recent["ema21"].iloc[-1]); e55=float(recent["ema55"].iloc[-1])
    ...
    if bear_aligned and atr_exp and not tight: return "trending_bear"
    if bull_aligned and atr_exp and not tight: return "trending_bull"
...
def detect_regime(df, df_4h_ext=None, df_1d_ext=None):
    bull_votes=0; bear_votes=0; ranging_votes=0; reasons=[]
    ...
    ema50_ref =d_ema50_price  if d_ema50_price>0  else ema50_now
    ema200_ref=d_ema200_price if d_ema200_price>0 else ema200_now

    # S1: EMA cross
    if ema50_ref>ema200_ref:
        bull_votes+=2; reasons.append(f"[S1] ✅ Golden cross EMA50>${ema50_ref:,.0f} > EMA200${ema200_ref:,.0f} → +2 bull")
        if d_ema200_slope>0.003: bull_votes+=1; reasons.append(f"[S1+] EMA200 rising → +1 bull")
    else:
        bear_votes+=2; reasons.append(f"[S1] ❌ Death cross EMA50${ema50_ref:,.0f} < EMA200${ema200_ref:,.0f} → +2 bear")
        if d_ema200_slope<-0.003: bear_votes+=1; reasons.append(f"[S1+] EMA200 falling → +1 bear")

    # S2: Price vs EMAs
    if price_now>ema50_ref and price_now>ema200_ref:
        bull_votes+=1; reasons.append("[S2] ✅ Price above both 1D EMAs → +1 bull")
    elif price_now<ema50_ref and price_now<ema200_ref:
    ...
    if df_4h_ext is not None and not df_4h_ext.empty and len(df_4h_ext)>=55:
        e21_4h=float(_ema(df_4h_ext["close"],21).iloc[-1]); e55_4h=float(_ema(df_4h_ext["close"],55).iloc[-1])
        if e21_4h>e55_4h: bull_votes+=1; reasons.append(f"[S3] ✅ 4H EMA21>${e21_4h:,.0f} > EMA55${e55_4h:,.0f} → +1 bull")
        else: bear_votes+=1; reasons.append(f"[S3] ❌ 4H EMA21 < EMA55 → +1 bear")
    else:
        h4b=bool(df["bias_bullish_4h"].iloc[-1]) if "bias_bullish_4h" in df.columns else price_now>ema50_now
        if h4b: bull_votes+=1; reasons.append("[S3] ✅ 4H bias bullish → +1 bull")
        else: bear_votes+=1; reasons.append("[S3] ❌ 4H bias bearish → +1 bear")

    # S4: Price structure
    structure=price_structure(df_1d_ext if df_1d_ext is not None else df, lookback=30)
    if structure=="bull": bull_votes+=1; reasons.append("[S4] ✅ HH+HL → +1 bull")
    elif structure=="bear": bear_votes+=1; reasons.append("[S4] ❌ LH+LL → +1 bear")
    else: ranging_votes+=1; reasons.append(f"[S4] ↔️ {structure} → +1 ranging")

    # S5: ADX
    adx=adx_strength(df_1d_ext if df_1d_ext is not None else df)
    if adx>25:
        if bull_votes>bear_votes: bull_votes+=1; reasons.append(f"[S5] ✅ ADX={adx:.1f} → +1 bull")
        elif bear_votes>bull_votes: bear_votes+=1; reasons.append(f"[S5] ✅ ADX={adx:.1f} → +1 bear")
        else: reasons.append(f"[S5] ADX={adx:.1f} — tied")
    elif adx<20: ranging_votes+=2; reasons.append(f"[S5] ↔️ ADX={adx:.1f} no trend → +2 ranging")
    else: ranging_votes+=1; reasons.append(f"[S5] ↔️ ADX={adx:.1f} weak → +1 ranging")
    ...
    above_vah=bool(df["above_vah"].iloc[-1]) if "above_vah" in df.columns else False
    if bull_votes>=5 and above_vah: regime="STRONG_BULL"
    elif bull_votes>=4: regime="STRONG_BULL"
    elif bull_votes>=2 and bull_votes>bear_votes and ranging_votes<3:
        regime="STRONG_BULL" if (bull_votes>=3 and price_now>ema50_ref and above_vah) else "WEAK_BULL"
    elif bear_votes>=4: regime="BEAR"
    elif bear_votes>=2 and bear_votes>bull_votes and ranging_votes<3: regime="BEAR"
    elif ranging_votes>=3 or (ranging_votes>=2 and adx<22): regime="RANGING"
    else: regime="WEAK_BULL" if ema50_ref>ema200_ref else "BEAR"

    reasons.append(f"🏷️ → {regime}")
    return regime, reasons

def get_regime_config(regime):
    base=dict(REGIME_CONFIGS.get(regime, REGIME_CONFIGS["WEAK_BULL"]))
    w=dict(CAT_WEIGHTS); w.update(base.get("weights_override",{}))
    base["weights"]=w
    return base
...
def compute_scores_regime(df, regime, min_cats, trend_req, h1_structure=None, h4_structure=None):
    ...
    be=(df["ema8"]>df["ema21"])&(df["ema21"]>df["ema55"])
    bb=(df["ema8"]<df["ema21"])&(df["ema21"]<df["ema55"])
    ...
    # ── Bear ranging override ──────────────────────────────────────
    h1r=h1_structure in ("ranging","recovering")
    h4r=h4_structure in ("ranging","recovering")
    mr_mod =(df["rsi"]<42)|df["stoch_oversold"]|df["mfi_oversold"]
    mr_str =(df["rsi"]<36)|(df["stoch_oversold"]&(df["rsi"]<42))
    at_sup =df["near_val"]|df["near_support"]|df["in_bullish_ob"]|df["near_fib618"]|df["near_poc"]|df["near_fib50"]
...
# ══════════════════════════════════════════════════════════════════
# RECOMMENDATION ENGINE — always provides guidance even with no signal
# ══════════════════════════════════════════════════════════════════

def get_recommendation(row, regime, h1_structure, h4_structure, long_score, short_score, rcfg):
    """
    Returns (action, bias, reasoning, key_levels) always — even when score < threshold.
    ...
    above_vah = bool(row.get("above_vah", False))
    near_support = bool(row.get("near_support", False))
    near_resistance = bool(row.get("near_resistance", False))
    min_long  = rcfg["min_score_long"]
    min_short = rcfg["min_score_short"]
    bear_ov   = row._parent.attrs.get("bear_override","NONE") if hasattr(row,"_parent") else "NONE"

    # Calculate how close we are to threshold
    long_gap  = min_long  - long_score
    short_gap = min_short - short_score
    ...
    if bool(row.get("long_signal", False)):
        action = "STRONG_BUY" if regime in ("STRONG_BULL","WEAK_BULL") else "BUY"
        bias   = "BULLISH"
        reasons.append(f"✅ Score {long_score:.1f} ≥ threshold {min_long}")
    elif bool(row.get("short_signal", False)):
        action = "STRONG_SELL" if regime=="BEAR" else "SELL"
        bias   = "BEARISH"
        reasons.append(f"✅ Short score {short_score:.1f} ≥ threshold {min_short}")

    # 2. Near-signal guidance
    elif long_gap <= 2.0 and long_gap > 0:
        action = "BUY"
        bias   = "BULLISH"
        reasons.append(f"⚡ Long score {long_score:.1f} is {long_gap:.1f}pts below threshold — close to BUY")
        if near_support: reasons.append("📍 Price near support — watch for confirmation")

    elif short_gap <= 2.0 and short_gap > 0 and regime in ("BEAR","RANGING"):
        action = "SELL"
        bias   = "BEARISH"
        reasons.append(f"⚡ Short score {short_score:.1f} is {short_gap:.1f}pts below threshold — close to SELL")
        if near_resistance: reasons.append("📍 Price near resistance — watch for confirmation")

    # 3. Market state guidance
    else:
        if regime == "BEAR":
            action = "HOLD"
            bias   = "BEARISH"
            reasons.append("🐻 Bear market — hold cash, wait for MR long setup at key support")
            if rsi_v < 35: reasons.append(f"⚡ RSI oversold ({rsi_v:.0f}) — watch for reversal signals")
            if mfi_v < 25: reasons.append(f"⚡ MFI oversold ({mfi_v:.0f}) — potential bounce zone")

        elif regime in ("STRONG_BULL","WEAK_BULL"):
            action = "HOLD" if not above_vah else "BUY"
            bias   = "BULLISH"
            if above_vah:
                reasons.append("🟢 Above VAH — bullish momentum, wait for pullback to EMA55/POC")
            else:
                reasons.append("📈 Bull trend — wait for dip to support before entering long")
            if rsi_v > 70: reasons.append(f"⚡ RSI overbought ({rsi_v:.0f}) — don't chase, wait for reset")

        elif regime == "RANGING":
            if near_support and rsi_v < 45:
                action = "BUY"; bias = "BULLISH"
                reasons.append("↔️ Ranging — at support with low RSI — potential bounce")
            elif near_resistance and rsi_v > 55:
                action = "SELL"; bias = "BEARISH"
                reasons.append("↔️ Ranging — at resistance with high RSI — potential fade")
            else:
                action = "HOLD"; bias = "NEUTRAL"
                reasons.append("↔️ Ranging market — wait for price to reach S/R extremes")

    # Oversold/overbought alerts regardless of regime
    if rsi_v < 30 and action in ("HOLD","WAIT"):
        reasons.append(f"🚨 RSI={rsi_v:.0f} — extreme oversold, high bounce probability")
        action = "BUY" if bias != "BEARISH" else action
    if rsi_v > 75 and action in ("HOLD","WAIT"):
        reasons.append(f"🚨 RSI={rsi_v:.0f} — extreme overbought, caution on new longs")
    ...
    return {
        ...
        "min_long_threshold": min_long,
        "min_short_threshold": min_short,
        "long_gap": round(long_gap, 2),
        "short_gap": round(short_gap, 2),
        "pct_to_long_signal": round(min(long_score/min_long*100, 100), 1),
        "pct_to_short_signal": round(min(short_score/min_short*100, 100), 1),
    }
...
def get_signal():
    ...
    if df_1h.empty:
        return {"error": "Could not fetch OHLCV data — check API key and network"}
    ...
    if direction in ("LONG","SHORT"):
        ...
        entry_plan={"entry":round(entry,2),"stop_loss":round(sl,2),"take_profit":round(tp,2),"rr":round(rr,1)}

    # Get recommendation (always populated)
    # Pass attrs through a simple wrapper
    class _RowProxy:
        def __init__(self, row, df):
            self._row = row; self._parent = df
        def get(self, k, d=None): return self._row.get(k, d)
        def __getitem__(self, k): return self._row[k]

    rec = get_recommendation(_RowProxy(row, df), regime, h1_str, h4_str, ls, ss, rcfg)
    ...
    indicator_map=[
        ...
        (f"RSI {rsi_v:.0f}", bool(rsi_v<42), bool(rsi_v>62)),
    ...
    # ── Intelligence layer (graceful) ─────────────────────────────
    intel={}
    if INTELLIGENCE_AVAILABLE:
        try:
            ep=entry_plan or {}
            intel=run_intelligence_layer(
                signal_direction=direction, tech_score=ls if lsig else ss,
                regime=regime, btc_price=price, ema50=ema50_v, ema200=ema200_v,
                ...
                account_value=INIT_CASH, run_research=True, run_polymarket=True,
                timeout_seconds=35,
            )
        except Exception as e:
            intel={"errors":[str(e)]}
    ...
    return {
        ...
        "indicators": {
            ...
            "poc":round(poc_v,2),"vah":round(vah_v,2),"val":round(val_v,2),
        },
        "intelligence":       intel,
...
def run_backtest(period="3M", tf="1H", capital=10000.0):
    period_days={"1M":30,"3M":90,"6M":180,"1Y":365}.get(period,90)
    tf_map={"1H":("hour",1,"1h"),"4H":("hour",4,"4h"),"1D":("day",1,"1d")}
    unit,agg,interval=tf_map.get(tf,("hour",1,"1h"))
    fetch_days=period_days+30

    df_main=fetch_ohlcv(unit,agg,days_back=fetch_days)
    if df_main.empty: return {"error":"No OHLCV data"}
    df_4h=fetch_ohlcv("hour",4,days_back=fetch_days+10)
    df_1d=fetch_ohlcv("day", 1,days_back=fetch_days+10)
    df_ind=compute_indicators(df_main,df_4h,df_1d,pd.DataFrame(),pd.DataFrame(),pd.DataFrame())
    ...
    sl_mult=rcfg.get("atr_sl_mult",ATR_SL); tp_mult=rcfg.get("atr_tp_mult",ATR_TP)
    cutoff=df_ind.index[-1]-pd.Timedelta(days=period_days)
    df_bt=df_ind[df_ind.index>=cutoff].copy()
    if len(df_bt)<10: return {"error":f"Only {len(df_bt)} bars after trim"}

    equity=capital; equity_curve=[capital]; trades=[]; in_trade=False
    entry_price=stop_loss=take_profit=0.0; direction="NONE"

    for i,(ts,row) in enumerate(df_bt.iterrows()):
        close=float(row["close"]); atr=float(row.get("atr",close*0.01))
        if in_trade:
            hit_sl=(direction=="LONG" and close<=stop_loss)or(direction=="SHORT" and close>=stop_loss)
            hit_tp=(direction=="LONG" and close>=take_profit)or(direction=="SHORT" and close<=take_profit)
            if hit_sl or hit_tp:
                exit_price=stop_loss if hit_sl else take_profit
                pnl_pct=(exit_price-entry_price)/entry_price
                if direction=="SHORT": pnl_pct=-pnl_pct
                pnl_usd=equity*0.10*pnl_pct
                equity=max(equity+pnl_usd,0)
                trades.append({"ts":str(ts)[:16],"direction":direction,"entry":round(entry_price,2),
                                "exit":round(exit_price,2),"pnl_pct":round(pnl_pct,4),
                                "pnl_usd":round(pnl_usd,2),"result":"WIN" if pnl_pct>0 else "LOSS",
                                "exit_type":"TP" if hit_tp else "SL"})
                in_trade=False; equity_curve.append(round(equity,2))
        if not in_trade:
            if bool(row.get("long_signal",False)): direction="LONG"
            elif bool(row.get("short_signal",False)): direction="SHORT"
            else: direction="NONE"
            if direction!="NONE":
                entry_price=close
                stop_loss  =close-atr*sl_mult if direction=="LONG" else close+atr*sl_mult
                take_profit=close+atr*tp_mult if direction=="LONG" else close-atr*tp_mult
                in_trade=True

    total_return=(equity-capital)/capital; n=len(trades)
    wins=[t for t in trades if t["result"]=="WIN"]; win_rate=len(wins)/n if n>0 else 0
    eq_arr=np.array(equity_curve); rets=np.diff(eq_arr)/np.maximum(eq_arr[:-1],1e-9)
    sharpe=float(rets.mean()/rets.std()*np.sqrt(len(rets))) if len(rets)>1 and rets.std()>1e-9 else 0
    peak=np.maximum.accumulate(eq_arr); dd=(eq_arr-peak)/np.maximum(peak,1e-9)
    max_dd=float(dd.min())

    result={"period":period,"timeframe":tf,"regime":regime,
            "total_return":round(total_return,4),"sharpe":round(sharpe,3),
            "max_dd":round(max_dd,4),"num_trades":n,"win_rate":round(win_rate,4),
            "final_equity":round(equity,2),"equity_curve":equity_curve,"run_history":trades,
...
def load_memory():
    paths=[MEMORY_FILE, Path("btc_backtest_memory_v8.json"), Path("/tmp/btc_memory.json")]
    for p in paths:
        if p.exists():
            try:
...
def save_to_memory(result):
    ...
    # Try multiple paths in case of read-only fs
    for p in [MEMORY_FILE, Path("btc_backtest_memory_v8.json"), Path("/tmp/btc_memory.json")]:
        try:
            with open(p,"w") as f: json.dump(mem,f,indent=2,default=str)
            return len(mem)
        except: pass
    return len(mem)
...
def get_learning_summary():
    ...
    best=ranked[0]
    lines=[f"📚 Learning Report ({len(mem)} runs)","",
           f"🥇 Best: {best.get('period','')} {best.get('timeframe','')} | "
           f"Return {best.get('total_return',0):.1%} | Sharpe {best.get('sharpe_ratio',0):.3f}"]
    return "\n".join(lines)
...
def log_signal_to_sheets(sig):
    log_to_sheets({"type":"SIGNAL","timestamp":sig.get("timestamp",""),
                   "direction":sig.get("direction",""),"price":sig.get("price",0),
                   "regime":sig.get("regime",""),"long_score":sig.get("long_score",0),
                   "short_score":sig.get("short_score",0)})

def log_trade_to_sheets(trade):
    log_to_sheets({"type":"TRADE",**{k:v for k,v in trade.items() if not isinstance(v,(dict,list))}})
...
def _diagnose_fetch():
    ...
    for name,url,params in [
        ...
        ("binance_main","https://api.binance.com/api/v3/klines",
         {"symbol":"BTCUSDT","interval":"1h","limit":3}),
        ("cryptocompare","https://min-api.cryptocompare.com/data/v2/histohour",
         {"fsym":"BTC","tsym":"USD","limit":3}),
    ]:
        try:
            headers={}
            if "cryptocompare" in name and CC_API_KEY:
                headers["authorization"]=f"Apikey {CC_API_KEY}"
            r=requests.get(url,params=params,headers=headers,timeout=10)
            if r.status_code==200: result[name]="✅ OK"
            else: result[name]=f"❌ HTTP {r.status_code}"
        except Exception as e:
            result[name]=f"❌ {type(e).__name__}: {str(e)[:60]}"
    result["api_key_set"]=bool(CC_API_KEY and CC_API_KEY not in ("","YOUR_CRYPTOCOMPARE_KEY_HERE"))
    return result
