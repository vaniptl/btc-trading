...
core/signal_runner.py  ← this file lives in /core/

Imported by:
  - cron/hourly_signal.py   (sys.path → parent/core)
  - app.py                  (sys.path → parent/core)

v6: EMA50, Fib 0.5, RSI hidden div, CVD absorption,
    real buying pressure, S/R 1H/4H/1D, Binance primary,
    daily trade history, log_signal_to_sheets for cron.
"""
...
# ══════════════════════════════════════════════════════════════════
# S/R + FIBONACCI  (called by get_signal and app.py)
# ══════════════════════════════════════════════════════════════════

def find_sr_levels(df_tf, swing=5, label="1H"):
    if df_tf is None or df_tf.empty:
        return {"levels": [], "nearest_support": None,
                "nearest_resistance": None, "label": label}
    ...
    for i in range(s, len(df_tf) - s):
        if h_v[i] == max(h_v[i-s:i+s+1]):
            ph.append(round(float(h_v[i]), 2))
        if l_v[i] == min(l_v[i-s:i+s+1]):
            pl.append(round(float(l_v[i]), 2))
...
def compute_indicators(df_1h, df_4h, liq_df, fund_df, oi_df, df_1d=None):
    ...
    df["cvd"]  = (df["volume"] * (
        (df["close"] - df["low"]) / hl - (df["high"] - df["close"]) / hl
    )).cumsum()

    # CVD divergence
    cvd_v  = df["cvd"].values
    c_vals = df["close"].values
    cvd_bull = [False] * len(df)
    cvd_bear = [False] * len(df)
    ...
    # Fibonacci
    r100   = df.tail(100)
    sh_    = float(r100["high"].max())
    sl_    = float(r100["low"].min())
    diff_  = sh_ - sl_
    down_  = r100["high"].idxmax() < r100["low"].idxmin()
    fib382 = sl_ + diff_*0.382 if down_ else sh_ - diff_*0.382
    fib50  = sl_ + diff_*0.500 if down_ else sh_ - diff_*0.500
    fib618 = sl_ + diff_*0.618 if down_ else sh_ - diff_*0.618
    ...
    df.attrs["fib_high"]     = round(sh_, 2)
    df.attrs["fib_low"]      = round(sl_, 2)
    ...
    for i in range(2, len(df) - 2):
        ...
        if np.isnan(a) or a == 0:
            continue
        if closes_[i] < opens_[i] and closes_[i+2] - closes_[i] > 2.0 * a:
            df["in_bullish_ob"] |= (
                (df["close"] >= min(opens_[i], closes_[i])) &
                (df["close"] <= max(opens_[i], closes_[i]))
            )
        if closes_[i] > opens_[i] and closes_[i] - closes_[i+2] > 2.0 * a:
            df["in_bearish_ob"] |= (
                (df["close"] >= min(opens_[i], closes_[i])) &
                (df["close"] <= max(opens_[i], closes_[i]))
            )
    ...
    rsi_v                     = df["rsi"].values
    rsi_bull  = [False] * len(df)
    rsi_bear  = [False] * len(df)
    rsi_hbull = [False] * len(df)
    rsi_hbear = [False] * len(df)
...
def compute_scores(df, w=None, min_score=None, min_cats=None,
    ...
    df["short_score"] = ts + zs + ms_ + ps

    def gate(t, z, m, p):
        cats_active = (
            (t > 0).astype(int) + (z > 0).astype(int) +
    ...
    df["short_signal"] = gate(ts, zs, ms_, ps) & enable_shorts
    df["trend_l"] = tl;   df["trend_s"] = ts
    df["zone_l"]  = zl;   df["zone_s"]  = zs
    df["mom_l"]   = ml;   df["mom_s"]   = ms_
    df["pos_l"]   = pl;   df["pos_s"]   = ps
    return df


# ══════════════════════════════════════════════════════════════════
# LIVE SIGNAL  — called every hour by cron/hourly_signal.py
# ══════════════════════════════════════════════════════════════════

def get_signal():
    """
    Returns a dict with all signal data.
    Always includes 'direction', 'price', 'long_score', 'short_score',
    'categories', 'indicators', 'reasons_long', 'reasons_short',
    'entry_plan', 'sr', 'fibonacci', and 'df'.
    ...
    try:
        ...
        indicator_checks = [
            ...
            ("RSI Oversold",        bool(rsi_v < 40),                                                                     True,  False),
            ("RSI Overbought",      bool(rsi_v > 60),                                                                     False, True),
        ...
        # RSI zone label
        if is_up and 30 <= rsi_v <= 50:       rsi_zone = "Bull Buy Zone (30-50)"
        elif not is_up and 50 <= rsi_v <= 70:  rsi_zone = "Bear Sell Zone (50-70)"
        elif rsi_v < 30:                        rsi_zone = "Oversold"
        ...
        return {
            ...
            "sr":        {"1H": sr_1h, "4H": sr_4h, "1D": sr_1d},
            "fibonacci": {"1H": fib_1h, "4H": fib_4h, "1D": fib_1d},
            "df": df,
        }
...
# ══════════════════════════════════════════════════════════════════
# GOOGLE SHEETS LOGGING  ← fixes the cron AttributeError
# ══════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════
# DAILY TRADE HISTORY  — called by app.py after backtest
# ══════════════════════════════════════════════════════════════════
...
def build_daily_trade_history(pf, init_cash=None):
    ...
    def get_col(col_name, default=None):
        if col_name and col_name in cols:
            return raw[col_name]
        n = len(raw)
        return pd.Series([np.nan if default is None else default] * n, index=raw.index)
...
# ══════════════════════════════════════════════════════════════════
# MEMORY  — backtest run persistence
# ══════════════════════════════════════════════════════════════════
